...
class LeNetConvPoolLayer(object):
    ...
    def predict(self, new_data):
        ...
        """
        conv_out = conv.conv2d(input=new_data, filters=self.W, filter_shape=self.filter_shape)
        if self.non_linear=="tanh":
            conv_out_tanh = T.tanh(conv_out + self.b.dimshuffle('x', 0, 'x', 'x'))
            output = downsample.max_pool_2d(input=conv_out_tanh, ds=self.poolsize, ignore_border=True, mode=self.poolmode)
